"""
M?dulo para registrar acciones de usuarios en archivo log y base de datos
con clasificaci?n por niveles de afectaci?n.
"""
import logging
...
# Niveles de afectaci?n permitidos
NIVELES_AFECTACION = {
    'aviso': 'Avisos - Acciones informativas normales',
    'movimiento': 'Movimientos - Acciones de creaci?n, edici?n o eliminaci?n',
...
class ActionLogger:
    ...
    def _get_client_ip(self) -> str:
        """Obtener la IP del cliente desde la petici?n"""
        if request:
            # Intentar obtener IP real (puede estar detr?s de proxy)
    ...
    def log_action(self, 
        ...
        """
        Registrar una acci?n de usuario en log y base de datos
        
        Args:
            usuario: Nombre del usuario que realiza la acci?n
            accion: Tipo de acci?n realizada (ej: 'login', 'crear_noticia', 'eliminar_usuario')
            nivel: Nivel de afectaci?n ('aviso', 'movimiento', 'ataque')
            descripcion: Descripci?n detallada de la acci?n
            ip: Direcci?n IP del cliente (se obtiene autom?ticamente si no se proporciona)
        """
        # Validar nivel
        if nivel not in NIVELES_AFECTACION:
            logger.warning("Nivel no v?lido en log_action, usando 'movimiento' por defecto")
            nivel = 'movimiento'
        ...
        # No registrar datos controlados por el usuario en logs (solo metadatos seguros)
        logger.info(
            "[ACCION] Registro de auditor?a (nivel=%s, longitud_acci?n=%s, tiene_descripci?n=%s)",
            nivel,
            len(accion) if accion else 0,
            bool(descripcion),
        )
        ...
        try:
            ...
            logger.error(f"Error al registrar acci?n en base de datos: {e}")
    ...
    def get_actions(self, 
        ...
        Args:
            ...
            nivel: Filtrar por nivel de afectaci?n
    ...
    def get_actions_count(self, nivel: Optional[str] = None, usuario: Optional[str] = None) -> int:
        """Obtener el total de acciones registradas (para paginaci?n)"""
        try:
            where_clauses = []
            params = []
